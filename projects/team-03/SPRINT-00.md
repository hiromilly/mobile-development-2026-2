# SPRINT 00 — Development Environment

## Team Information
- **Team:** Team 03
- **Members:**
  - Célia Hiromi (@hiromilly)
  - Geovanna Gaspar] (@gegwspar)

## Development Environment Status
- **Android Studio:** Installed and configured
- **Android SDK:** Configured
- **Android Emulator / Device:** Configured and operational
- **Technology Stack:** Kotlin, Jetpack Compose, Gradle

## Difficulties Encountered
- Initial setup and alignment of Git/GitHub workflow (Fork, Upstream synchronization, and Pull Request submission).
- Environment cleanup and proper `.gitignore` configuration for Android Studio build artifacts.

## Acceptance Criteria
- [x] AC-01 — Team members are identified.
- [x] AC-02 — Git is correctly configured.
- [x] AC-03 — The course repository has been forked.
- [x] AC-04 — A Sprint branch has been created (`team-03/sprint-00`).
- [x] AC-05 — Android Studio is installed.
- [x] AC-06 — Android SDK is configured.
- [x] AC-07 — An Android Emulator or physical device is available.
- [x] AC-08 — An Android application builds successfully.
- [x] AC-09 — The application runs without crashing.
- [x] AC-10 — At least one commit has been created.
- [x] AC-11 — A Pull Request has been submitted.
